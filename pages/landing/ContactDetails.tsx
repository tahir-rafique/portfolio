...
import ParaText from "@/components/ui/Paratexts";

export default function ContactDetails() {
  return (
    <section aria-label="contact-details" className="flex flex-col gap-6">
      <TitleText title="Contact Details" icon={ContactRound} />
      <ParaText
      ...
      <div className="flex items-center gap-14">
        {LandingPageData.contactPlatforms.map((platform) => (
          <Link href={platform.href} key={platform.id}>
            <platform.icon className=" size-10" color="var(--primary)" />
          </Link>
        ))}
      </div>
    </section>
