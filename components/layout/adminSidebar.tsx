...
export default function AdminSidebar() {
  ...
  return (
    <aside
      ...
      } h-full w-full max-w-[300px] bg-indigo-100  max-lg:rounded-l-sm! rounded-4xl p-3 relative max-lg:fixed top-0 bottom-0  left-0 max-lg:shadow-md`}
    >
      <button
      ...
      <div className="flex flex-col gap-5 justify-between h-full max-h-[calc(100%-60px)]">
        <ul className="space-y-2 py-8">
          {AdminData.adminSidebarLinks.map((navlink) => {
            return (
              <li
                key={navlink.id}
                className={`text-xl font-inter leading-[140%] rounded-primary p-2 flex items-center gap-3 cursor-pointer transition-all duration-75 ${
                  adminNavLink === navlink.label
                    ? "bg-indigo-200 font-medium"
                    : "hover:bg-indigo-200/20"
                }  ${
                  openAdminSidebar
        ...
        <p className="text-center text-[clamp(0.1rem,2vw,0.9rem)] font-satoshi font-medium">
          Created by @Tahir!
        </p>
      </div>
    </aside>
